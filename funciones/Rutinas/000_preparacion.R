preparacion <- function(sec_man,eje,adicio){
  #SECTOR_MANZANA_A=st_read("D:/ejercicio 4 Isa/SECTOR_MANZANA_A.shp") %>% 
  #st_cast("MULTIPOLYGON")  
  #Ejes=st_read("D:/ejercicio 4 Isa/ejes.shp") %>% 
  #st_as_sf() %>%
  # st_cast("LINESTRING") %>% 
  # group_by() %>% 
  # summarise()
  #adicionales=st_read("D:/ejercicio 4 Isa/ADICIONAL_L.shp") %>% 
  #st_as_sf() %>% 
  #st_cast("LINESTRING") %>% 
  #  group_by() %>% 
  #  summarise()
...
buffer = st_union(SECTOR_MANZANA_A_l,Ejes,adicionales)%>% 
  ...
  st_simplify() %>% 
  st_buffer(6)
#st_write(SECTOR_MANZANA_A,"D:/ejercicio 4 Isa/borrador/resu10.shp")
buffer=st_segmentize(buffer, 2) 
...
vorpun <- puntos %>% 
  ...
  group_by() %>% 
  summarise() 
buffer=buffer %>% st_simplify(dTolerance = 2)
...
man_cent=st_centroid_within_poly(SECTOR_MANZANA_A)%>% 
  st_as_sf()%>% 
  st_cast("MULTIPOINT")
...
new_man= st_join(MANZANAS,man_cent, largest = TRUE) %>% filter(!is.na(id_ae))
